...
import { GetLoans } from "../../helpers/api";
...
export const HistoricalLoans = () => {
    ...
    const handleGetLoans = async () => {
        ...
        try{
            const response = await GetLoans(session?.id);
  ...
  return (
    <UserLandingWrapper>
      <div className=" justify-content-center p-6">
        ...
        >
            <Column field="equipmentName" header="Nombre del equipo"  align={"center"}/>
            <Column 
                field="startDate" 
                header="Fecha de inicio" 
                align="center" 
                body={(rowData) => new Date(rowData.startDate).toISOString().replace("T", " ").substring(0, 16)} 
            />
            <Column 
                field="endDate" 
                header="Fecha finalización" 
                align="center" 
                body={(rowData) => new Date(rowData.endDate).toISOString().replace("T", " ").substring(0, 16)} 
            />

            <Column field="status" header="Estado" align={"center"} body={(rowData) => renderTemplate(rowData.status)} />
