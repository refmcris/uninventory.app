...
import { GetActiveInactiveLoans } from "../../helpers/api";
...
export const ActiveInactiveLoansScreen = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await GetActiveInactiveLoans();
        if (data) {
          setChartData(data);
          setChartOptions({
            plugins: {
        ...
        console.error("Error al cargar datos del endpoint /activeInactiveLoans:", err);
        setChartData(defaultData);
        setChartOptions(defaultOptions);
        setError(true);
      }
  ...
  return (
    <UserLandingWrapper>
      ...
      <div className="p-4" style={{ marginTop: '20px' }}>
        <h2 className="text-3xl font-bold mb-4">Prestamos Activos vs Inactivos</h2>
        {error && (
          <p className="text-red-500 mb-3 font-semibold">
            ⚠️ No se pudieron cargar los datos del endpoint /activeInactiveLoans. Se muestran datos por defecto.
          </p>
        )}
        {chartData && chartOptions ? (
          <ActiveInactiveLoansChart data={chartData} options={chartOptions} />
        ) : (
