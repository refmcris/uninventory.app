  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error("Error al cargar datos del endpoint /activeInactiveEquipments:", err);
        setChartData(defaultData);
        setChartOptions(defaultOptions);
        setError(true);
      }
      ...
      <div className="p-4" style={{ marginTop: '20px' }}>
        ...
        {error && (
          <p className="text-red-500 mb-3 font-semibold">
            ⚠️ No se pudieron cargar los datos del endpoint /activeInactiveEquipments. Se muestran datos por defecto.
          </p>
        )}
        {chartData && chartOptions ? (
          <ActiveInactiveEquipmentsChart data={chartData} options={chartOptions} />
        ) : (
